...
procedures that aren't Snap<em>!</em> primitives, but are provided as
...
(world). Snap<em>!</em> inherits its text handling from Scratch.
...
them. In Snap<em>!</em>, the visual representation of a list looks nothing at
...
Costumes are first class data in Snap<em>!</em>.
...
a better alternative to the HSV that Snap<em>!</em> inherits from JavaScript; a
...
All of these could be written in Snap<em>!</em>
...
one Snap<em>!</em> session to the next, if you use the same browser and the
same login.
...
The **SciSnap<em>!</em> library** and the **TuneScope library** are too big to
